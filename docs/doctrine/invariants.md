...
| `INV-HTTP-003` | Only the timestamped mode has a replay window. The body-HMAC and static-header modes accept a valid credential indefinitely. | [`architecture/http-surface.md`](../architecture/http-surface.md) |
...
| `INV-HTTP-005` | Every external entry point must be able to say who spoke. The ingress-identity table is validated at boot against an independently-written contract, and a route that cannot name its speaker is a boot failure. | [`architecture/http-surface.md`](../architecture/http-surface.md) |
