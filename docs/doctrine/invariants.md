...
| `INV-DOC-002` | An architecture document follows one section order — Scope, Mental model, Contracts & invariants, Failure behavior, Extension points, Source & test map — so an agent can skim positionally; doctrine and contributing documents keep the code-wins line and the source map but shape their own sections. | [`contributing/doc-contract.md`](../contributing/doc-contract.md) |
