...
def test_bundletest_component_loads_with_available_dependencies_including_its_plugin() -> None:
    """Rot-check for the Task 13 e2e fixture
    (test_bundled_specialist_install.sh) — the committed
    ``bundletest`` component, generated by ``gen_fixtures.py``'s
    ``write_bundled_specialist_component``, must keep resolving its bundled
    ``bt`` plugin dependency as available. A schema/canonicalization/
    plugin_store drift would otherwise only surface opaquely inside the
    Docker e2e."""
    component = load_specialist_component(_BUNDLED_COMPONENT, _BUNDLED_COMPONENT / "manifest.json")
    assert component.slug == "bundletest"
    assert component.component_id == "casa-test/bundletest"
    assert component.version == "0.1.0"
    plugin_deps = [d for d in component.dependencies if d.kind == "plugin/implementation"]
    assert len(plugin_deps) == 1
    assert plugin_deps[0].identifier == "bt"
    assert plugin_deps[0].source is not None and plugin_deps[0].source.type == "bundled"
    assert plugin_deps[0].source.path == "plugins/bt"

    deps = resolve_dependency_closure(component, _BUNDLED_COMPONENT)
    assert deps, "fixture must declare its bundled persona + plugin dependencies"
    assert all(d.available for d in deps), [
        (d.kind, d.identifier, d.detail) for d in deps if not d.available
    ]
